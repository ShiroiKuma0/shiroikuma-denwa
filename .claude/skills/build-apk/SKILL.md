...
This app builds against our patched Fossify Commons (`commons = "6.1.6-sk7"` in
...
with `Could not resolve org.fossify:commons:6.1.6-sk7` — publish it first:

```bash
cd ~/git/shiroikuma-commons && JAVA_HOME=/usr/lib/jvm/java-21-openjdk-amd64 \
  ./gradlew :commons:publishToMavenLocal -PVERSION=6.1.6-sk7
```
